...
(1) \textbf{Conceptual Design:} User flow diagrams were created for the two primary workflows---manual authoring and AI-assisted generation. These flows were reviewed against Nielsen's heuristics to identify sources of extraneous cognitive load before any wireframing began.
(2) \textbf{Wireframing and Heuristic Evaluation:} High-fidelity wireframes were built in Figma and evaluated against three heuristics: visibility of system status (ensuring loading indicators were present during AI generation), error prevention (disabling submit buttons until validation passes), and recognition over recall (displaying the transcript text alongside the video so instructors don't need to memorize dialogue).
(3) \textbf{Prototype Testing:} The interactive Figma prototype was tested with two senior IT students using a think-aloud protocol. This revealed three usability problems: timestamp fields weren't auto-populated when adding a new question; AI-related status messages were too subtle to notice; and button labels on the injection panel were ambiguous. All three were fixed before formal evaluation. The resulting workflow reduced the total number of interface interactions needed to author a three-question interactive video from 47 (in the legacy WordPress baseline) to 8 in the new platform.
...
The most technically interesting part of the frontend is how it handles the AI generation process. LLM generation can take upwards of 15 seconds, and waiting for a synchronous HTTP response would freeze the UI. The frontend uses the native \texttt{EventSource} API to establish an SSE connection with the backend instead. As questions are validated and enriched on the backend, they stream individually to the UI, appearing one at a time rather than all at once. From a user experience perspective, this makes the system feel responsive and active---the instructor can start reviewing the first questions while the rest are still being generated.
...
\begin{itemize}
  ...
  \item \textbf{Decoupled Content CRUD.} H5P content parameters are stored as clean JSON documents in \texttt{h5p-content/}, indexed by UUID, rather than tangled in a CMS database.
...
Each accepted suggestion is built into a content object by \texttt{buildContentData()}, which resolves the H5P library identifier from \texttt{H5P\_TYPE\_MAP} and constructs the content metadata:
...
After iterating through all accepted suggestions, the function performs a \textbf{single database write} (one \texttt{UPDATE} on the video row). The alternative---issuing one database write per suggestion---would produce N separate SQL UPDATE statements for N accepted suggestions. The single-write approach reduces database round-trips to one regardless of how many suggestions are accepted, which matters when an instructor accepts a full batch of 6--10 AI-generated questions at once.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth,height=0.65\textheight,keepaspectratio]{images/Authentication Sequence.jpeg}
...
The pedagogical quality of the AI-generated suggestions was evaluated by having the project supervisor---acting as an expert instructional design rater---blind-rate 30 randomly selected AI-generated suggestions from three diverse videos. Three criteria were used:
...
\begin{itemize}
  ...
  \item \textbf{SQLite read concurrency.} SQLite uses a shared-reader / exclusive-writer locking model. Read operations (fetching video lists, loading H5P content) run concurrently without blocking each other, which covers the majority of requests during normal browsing.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.65\textheight,keepaspectratio]{images/homepagelight.png}
  \caption{Login page}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.65\textheight,keepaspectratio]{images/Home page.png}
  \caption{Home page}
  \label{fig:home_page}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.65\textheight,keepaspectratio]{images/Upload page.png}
  \caption{Upload page}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.65\textheight,keepaspectratio]{images/Editor page.png}
  \caption{Editor page}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.65\textheight,keepaspectratio]{images/Question types menu.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.65\textheight,keepaspectratio]{images/UI when editing an interaction.png}
  \caption{Editing an interaction}
  \label{fig:editing_interaction}
\end{figure}

% Correct / incorrect kept side-by-side for direct visual comparison
\begin{figure}[H]
  \centering
  \begin{minipage}[t]{0.47\linewidth}
    \centering
    \includegraphics[width=\linewidth]{images/Answered correct.png}
    \caption{Correct answer feedback}
    \label{fig:answer_correct}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.47\linewidth}
    \centering
    \includegraphics[width=\linewidth]{images/Answered incorrect.png}
    \caption{Incorrect answer feedback}
    \label{fig:answer_incorrect}
  \end{minipage}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.65\textheight,keepaspectratio]{chapters/Admin Console page.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.65\textheight,keepaspectratio]{images/Profile page.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.65\textheight,keepaspectratio]{images/Settings page.png}
