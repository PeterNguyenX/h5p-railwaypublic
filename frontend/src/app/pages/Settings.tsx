...
const LANGUAGES = [
  { code: "en", labelKey: "language.english", fallback: "English" },
  { code: "vi", labelKey: "language.vietnamese", fallback: "Tiếng Việt" },
];
...
export default function Settings() {
  ...
  const languageOptions = LANGUAGES.map((languageOption) => ({
    ...
    label: t(languageOption.labelKey, languageOption.fallback),
  }));
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#1a1a1a] flex flex-col transition-colors duration-200">
      <header className="bg-white dark:bg-[#242424] border-b border-slate-200 dark:border-white/10 shadow-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <span className="text-base font-semibold text-slate-800 dark:text-gray-200">{t('settings.title', 'Settings')}</span>
          <button
            type="button"
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full space-y-6">
        ...
        <section className="bg-white dark:bg-[#242424] rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-200 dark:border-white/10">
            <h2 className="text-sm font-bold text-slate-500 dark:text-gray-400 uppercase tracking-widest">{t('settings.appearance', 'Appearance')}</h2>
          </div>
          <div className="divide-y divide-slate-100 dark:divide-white/5">
            <div className="flex items-center justify-between px-6 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-800 dark:text-gray-200">{t('settings.darkMode', 'Dark Mode')}</p>
                  <p className="text-xs text-slate-500 dark:text-gray-400">{t('settings.darkModeDesc', 'Switch to a dark colour theme')}</p>
                </div>
              </div>
        ...
        <section className="bg-white dark:bg-[#242424] rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-200 dark:border-white/10">
            <h2 className="text-sm font-bold text-slate-500 dark:text-gray-400 uppercase tracking-widest">{t('settings.languageRegion', 'Language & Region')}</h2>
          </div>
          <div className="px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-800 dark:text-gray-200">{t('settings.interfaceLanguage', 'Interface Language')}</p>
                <p className="text-xs text-slate-500 dark:text-gray-400">{t('settings.interfaceLanguageDesc', 'Choose your preferred display language')}</p>
              </div>
            </div>
            <select
              value={language}
        ...
        <section className="bg-white dark:bg-[#242424] rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-200 dark:border-white/10">
            <h2 className="text-sm font-bold text-slate-500 dark:text-gray-400 uppercase tracking-widest">{t('settings.reset', 'Reset')}</h2>
          </div>
          <div className="px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-800 dark:text-gray-200">{t('settings.resetDefaults', 'Reset to Defaults')}</p>
                <p className="text-xs text-slate-500 dark:text-gray-400">{t('settings.resetDesc', 'Restore all settings to their default values')}</p>
              </div>
            </div>
            <button
              type="button"
