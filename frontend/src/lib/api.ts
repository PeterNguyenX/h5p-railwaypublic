...
export async function autoTranscribe(videoId: string): Promise<{ segments: TranscriptSegment[]; source: string; message: string }> {
  const res = await fetch(`${API_BASE}/transcript/whisper/${videoId}`, {
    method: 'POST',
    headers: authHeaders(),
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.error || 'Auto-transcription failed');
  }
  return res.json();
}

export async function extractVideoTranscript(videoId: string): Promise<TranscriptSegment[]> {
...
export type H5PInteractionType = 'MultiChoice' | 'TrueFalse' | 'FillBlanks' | 'Hotspot' | 'DragDrop' | 'MarkWords';
...
export async function createH5PContent(
  ...
  // Reuse the inject endpoint by sending a single accepted suggestion
  const library: Record<H5PInteractionType, string> = {
  ...
  const suggestion = {
    ...
    timestamp,
    type,
    h5pLibrary: library[type],
    config,
    reason: 'Manually added',
    status: 'accepted' as const,
  };
  const result = await injectSuggestions([suggestion], videoId);
  if (result.errors.length > 0 && result.injected.length === 0) {
    throw new Error(result.errors[0]?.error || 'Failed to create H5P content');
  }
  // Return a compatible H5PContent object
  const injected = result.injected[0];
  return {
    id: injected.contentId,
    library: injected.library,
    params: config,
    metadata: { title: type, license: 'U' },
