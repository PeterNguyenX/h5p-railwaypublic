...
The chapter presents the research design, requirements analysis process, user-centred design methodology, agile development approach, and the empirical evaluation protocol. The second half details the AI pipeline architecture: the prompt engineering strategy, the pattern matrix used for question type selection, and the Zod validation approach.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1\linewidth,height=0.42\textheight,keepaspectratio]{images/Usecase.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,height=0.5\textheight]{images/three-tier-system.jpeg}
...
The AI pipeline transforms a raw, timestamped video transcript into a validated, Bloom's taxonomy-labelled list of H5P interaction JSON configurations. The transformation passes through five stages. Figure~\ref{fig:ai_pipeline} shows the overall data flow, and Figure~\ref{fig:seq_ai} shows the detailed sequence diagram including the SSE streaming path.

\begin{figure}[H]
  \centering
\includegraphics[width=1\linewidth,height=0.9\textheight,keepaspectratio]{images/AI Pipeline.jpeg}
...
\begin{figure}[H]
  \centering
 \includegraphics[width=1\linewidth,height=0.95\textheight,keepaspectratio]{images/AI asisted question generation.jpeg}
...
The compact \texttt{[Xs-Ys]} timestamp notation saves roughly 22 characters per line compared to a verbose human-readable format, reducing the total token count by approximately 600 tokens for a 100-segment transcript. Raw-second values are used directly in the JSON output so the model never needs to perform time arithmetic, which eliminates a common source of timestamp hallucination~\cite{white2023}.
...
The core AI system prompt, defined as the constant \texttt{AI\_SYSTEM\_PROMPT} in \texttt{aiService.js}, implements a two-step \emph{output automater} prompting pattern~\cite{white2023}. Separating high-level topic discovery from the mechanical task of generating JSON significantly improves output consistency, because the model's attention at each step is narrowly focused.
...
Table~\ref{tab:pattern_examples} shows a representative AI-generated example for each question type, taken from actual pipeline output on a computer networks lecture transcript.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{
    ...
    \toprule
    \textbf{H5P type} & \textbf{Example AI-generated question (condensed)} \\
    \midrule
    FillBlanks    & ``The \texttt{*TCP/IP*} model divides network communication into four layers.'' \\
    \addlinespace
    TrueFalse     & ``UDP guarantees packet delivery order.'' $\rightarrow$ \texttt{correct: false} \\
    \addlinespace
    MultiChoice   & ``Which layer of the OSI model handles routing?'' Options: Network (correct), Transport, Session, Physical \\
    \addlinespace
    DragText      & Arrange the TCP handshake steps: \texttt{*SYN*}, \texttt{*SYN-ACK*}, \texttt{*ACK*} \\
    \addlinespace
    MarkWords     & ``In packet switching, \texttt{*routers*} forward \texttt{*datagrams*} based on the destination \texttt{*IP address*}.'' \\
    \addlinespace
    Matching      & Pair: HTTP $\leftrightarrow$ Application layer; TCP $\leftrightarrow$ Transport layer; IP $\leftrightarrow$ Network layer; Ethernet $\leftrightarrow$ Data link layer \\
    \bottomrule
  \end{tabularx}
  \caption{Representative AI-generated question examples per H5P type}
  \label{tab:pattern_examples}
\end{table}
